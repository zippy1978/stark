...
type EntryFunction = unsafe extern "C" fn() -> i32;
...
impl VirtualMachine {
    ...
    pub fn run_bitcode(&self, bitcode: &MemoryBuffer) -> Result<i32, VirtualMachineError> {
        match Module::parse_bitcode_from_buffer(bitcode, &self.context) {
            Ok(module) => {
                match module.create_jit_execution_engine(self.config.optimization_level) {
                    Ok(execution_engine) => unsafe {
                        let entry_function = execution_engine.get_function::<EntryFunction>("main");
                        Result::Ok(entry_function.unwrap().call())
                    },
